...
// Aurora WebGL component — ported from reactbits reference
// Uses OGL (WebGL library) for the animated simplex-noise aurora effect
...
export default function Aurora({
  colorStops = ["#6C5CE7", "#00CEC9", "#E84393"],
  amplitude = 1.0,
  blend = 0.5,
  speed = 1.0,
  className = "",
  ...
  useEffect(() => {
    ...
    import("ogl").then(({ Renderer, Program, Mesh, Color, Triangle }) => {
      ...
      // Cleanup
      (ctn as HTMLDivElement & { _auroraCleanup?: () => void })._auroraCleanup = () => {
    ...
    return () => {
      cancelAnimationFrame(animateId);
      const cleanup = (ctn as HTMLDivElement & { _auroraCleanup?: () => void })._auroraCleanup;
      cleanup?.();
    };
